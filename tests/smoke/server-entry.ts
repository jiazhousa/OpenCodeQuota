// smoke 专用 server 侧插件入口（V2 双侧架构，2.0.16）：
// 数据层在 server 侧（src/server.ts），mock 注入点随之迁移——本文件替换供应商 fetch 并审计请求；
// CLI 侧渲染入口（tests/smoke/entry.tsx）与生产完全一致，无任何 mock。
...
export default createQuotaServerPlugin({ fetch: mockFetch });
